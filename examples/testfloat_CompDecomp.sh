...
do
	echo processing $file
        echo testfloat_CompDecomp sz.config zc.config "sz($absErrBound)" "$file" $errBoundMode $absErrBound "$targetDir/$file" $dimSizes
        testfloat_CompDecomp sz.config zc.config "sz($absErrBound)" "$file" $errBoundMode $absErrBound "$targetDir/$file" $dimSizes
done
