...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < ITER_TIMES; i++) {
		...
		{	
			...
			sprintf(cmprCaseName, "%s(1E-2)", compressorName); //name the compression case
			...
			if(rank==0)
				printf("step %d : %f %f %f %f\n", i, compareResult->psnr, compareResult->compressRatio, compareResult->compressRate, compareResult->decompressRate);
