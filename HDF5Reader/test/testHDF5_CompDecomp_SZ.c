...
void usage()
{
	printf("Usage: testHDF5_CompDecomp_SZ <options>\n");
	printf("Options:\n");
	...
	printf("* examples:\n");
	printf("	testHDF5_CompDecomp_SZ -i testdata/testdata.h5 -d 2 -c ~/Development/Z-checker/z-checker-0.2/examples/zc.config -e ~/Development/z-checker-installer/z-checker-installer/errBounds.cfg\n");
}
...
int main(int argc, char * argv[])
{
	...
	char* errBoundString = iniparser_getstring(ini, szErrBoundKey, NULL);
...
//scan the hdf5 file to do the compression, decompression, and z-checker assessment
	...
	grp = H5Gopen(file,"/");
	scan_group_SZ(grp, errBoundMode, nbErrBounds, errBounds);
