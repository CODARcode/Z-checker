...
{	
	...
	if(subDir==NULL)
		sprintf(tmpLine, "\\raisebox{-1cm}{\\includegraphics[scale=.34]{figs/{%s}.png}}", epsFileName);
	else
		sprintf(tmpLine, "\\raisebox{-1cm}{\\includegraphics[scale=.34]{figs/%s/{%s}.png}}", subDir, epsFileName);	
