...
void zc_init_c_(char *configFile,int *len,int *ierr)
{
    int i;
    char s2[*len+1];
    for(i=0;i<*len;i++)
...
void zc_startcmpr_d1_float_(char* varName, float* data, size_t *r1)	
{
    int i;
    char s2[*len+1];
    for(i=0;i<*len;i++)
        s2[i]=varName[i];
    s2[*len]='\0';	
	ZC_DataProperty* property = ZC_startCmpr(ZC_FLOAT, data, outSize, 0, 0, 0, 0, *r1);

	//TODO
	//how to deal with property: the easiest way is to keep property in C space; specifically, we need to build a hashtable (key:varName, value:ZC_DataProperty*)
}
